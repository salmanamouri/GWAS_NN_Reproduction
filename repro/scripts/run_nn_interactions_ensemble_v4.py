...
def main():
    out_dir = Path("repro/outputs/nn_ensemble_v4")
    out_dir.mkdir(parents=True, exist_ok=True)

    device = "cuda" if torch.cuda.is_available() else "cpu"
    print("Using device:", device)

    # Stronger paper-like setting
    n_samples = 40000
    n_genes = 10
    snps_per_gene = 20
    causal_prop = 0.5
    ...
    with open(out_dir / "nn_ensemble_v4_results.json", "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)

    print("Ensemble V4 NN interaction metrics:", metrics)
    print("Top 10 pairs:")
